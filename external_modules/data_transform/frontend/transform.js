        ...
        try {
            ...
            switch (action) {
                ...
                case "prepare-from-splits":
                    if (!confirm("This will DELETE all full vector files (vectors.jsonl / text_data.jsonl) and rebuild them from split shards. Split files will NOT be deleted. Continue?")) {
                        this.poller.log("Rebuild from splits cancelled by user.");
                        return;
                    }
                ...
                case "scan-import":
                    ...
                    this.poller.log(`Scan: ${JSON.stringify(result.stats || result)}`);
                    break;
                default:
