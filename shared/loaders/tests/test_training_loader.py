...
def test_reset_and_remove_training_models(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    loader.processed_data = object()
    loader.filtered_data = (np.array([1.0]), np.array([0]))
    ...
    assert loader.filtered_data is None
...
def test_load_vectors_scores_and_cache(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    _write_jsonl(vectors_path, [[1, 2], [3, 4]])
    _write_jsonl(scores_path, [0.5, 0.9])
    ...
    _write_jsonl(vectors_path, [[9, 9]])
    _write_jsonl(scores_path, [1.0])
    assert loader.load_vectors().shape == (2, 2)
    assert loader.load_scores().tolist() == [0.5, 0.9]


def test_filtered_and_interaction_data_roundtrip(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    filtered_path = tmp_path / "filtered.npz"
    interaction_path = tmp_path / "interaction.npz"
    monkeypatch.setattr(training_loader, "filtered_data", str(filtered_path))
    ...
    saved_filtered = loader.save_filtered_data(x, kept)
    assert saved_filtered[0].shape == (1, 2)
    assert loader.load_filtered_data()[1].tolist() == [0]
