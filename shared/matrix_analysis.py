...
class MatrixAnalyzer:
    ...
    def __init__(
        self, scores: list[float], text_data: list[dict[str, Any]], memory_limit: int = 10000
    ):
        """
        Initialize analyzer.
    ...
    def _add_param_from_value(self, key: str, value: Any, params: list[str], prefix: str = "") -> None:
            ...
            for item in value:
                if isinstance(item, (str, int, float)):
                    ...
                    if isinstance(tuple_weight, (int, float)):
                        tuple_weight = round(tuple_weight * weight * weight2, 1)
    ...
    def calculate_statistics(self, min_count: int = 100) -> dict[tuple[int, int], dict[str, float]]:
        """Calculate statistics for all matrix cells with sufficient sample count."""
        ...
        with tqdm(total=total_possible_cells, desc="Flattening Matrix", unit="cells") as pbar:
        ...
        for row in tqdm(stats_df.iter_rows(named=True), total=len(stats_df), desc="Building Final Dict"):
            p1, p2 = row["p1"], row["p2"]
