...
def parse_entry_meta(
...
) -> Tuple[float, float, float, Any, Any, Any, Any, float, float, float]:
    ...
    width_val = float(entry["width"])
    height_val = float(entry["height"])
    ...
    w_max = float(normalization["width_max"]) if "width_max" in normalization else 4096.0
    h_max = float(normalization["height_max"]) if "height_max" in normalization else w_max
    ar_max = float(normalization["aspect_ratio_max"]) if "aspect_ratio_max" in normalization else 5.0

    width_norm = max(0.0, min(1.0, width_val / w_max))
    height_norm = max(0.0, min(1.0, height_val / h_max))
    ar_norm = max(0.0, min(1.0, ar_val / ar_max))
