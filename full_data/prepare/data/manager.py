...
def encode_image_to_vector(path: str) -> List[float]:
    ...
    # SigLIP / CLIP handling
    inputs = _clip_processor(images=img, return_tensors="pt")
    inputs = {k: v.to(_clip_model.device) for k, v in inputs.items()}

    with torch.no_grad():
        # SigLIP returns pooler_output or similar, usually accessible via direct call if it's AutoModel
        if hasattr(_clip_model, "get_image_features"):
            # CLIP
            outputs = _clip_model.get_image_features(**inputs)
        else:
            # SigLIP / DINOv2 generic AutoModel often returns last_hidden_state or pooler_output
            # For SigLIP, we want the image embeddings.
            # SigLIP's AutoModel is typically SiglipVisionModel, which might return a base output.
            # Let's assume standard behavior or explicit method if known.
            # Actually, google/siglip is usually loaded as SiglipModel (text+vision) or SiglipVisionModel.
            # AutoModel loads the Vision part if we only give it images? No, usually loads the whole thing if generic.
            # Let's be safe: SigLIP has `get_image_features` if we use the full model,
            # but if AutoModel loads `SiglipModel`, it behaves like CLIP.
            outputs = (
                _clip_model.get_image_features(**inputs)
                if hasattr(_clip_model, "get_image_features")
                else _clip_model(**inputs).pooler_output
            )

    vec = outputs[0].cpu().numpy().astype(float).tolist()
...
def pad_existing_vectors(
    vectors_file: str, schema: Dict[str, Any], prompt_representation: Optional[Dict[str, Any]] = None
) -> None:
